...
model_map = {'mlp_baseline':'MLP', 'full_gnn':'FullGNN', 'tiny_gnn':'TinyGNN'}
colors = {'MLP':'#4C78A8','FullGNN':'#F58518','TinyGNN':'#54A24B'}
df['Experimento'] = df['experiment_id'].map(exp_map)
df['Modelo'] = df['model'].map(model_map)
order_exp = [exp_map[k] for k in exp_map]
order_model = ['MLP','FullGNN','TinyGNN']
...
def grouped_bar(metric, ylabel, title, filename, log=False):
    ...
    x = np.arange(len(order_exp)); width = 0.24
...
markers = {'MLP':'o','FullGNN':'s','TinyGNN':'^'}
